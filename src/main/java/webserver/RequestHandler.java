    ...
    public void run() {
        ...
        try (InputStream in = connection.getInputStream(); OutputStream out = connection.getOutputStream()){
            ...
            if (method.equals("GET") && url.equals("/user/login.html")){
                File loginFile = new File("webapp/user/login.html");
                byte[] body = Files.readAllBytes(loginFile.toPath());
                response200Header(dos, body.length);
                responseBody(dos, body);
            }
            if (method.equals("POST") && url.equals("/user/login")){
                ...
                String loginUserInfo = IOUtils.readData(br, requestContentLength);
            ...
            if (method.equals("GET") && url.equals("/user/login_failed.html")){
                File loginFailFile = new File("webapp/user/login_failed.html");
                byte[] body = Files.readAllBytes(loginFailFile.toPath());
                response200Header(dos, body.length);
                responseBody(dos, body);
            }
